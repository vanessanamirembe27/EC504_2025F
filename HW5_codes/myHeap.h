...
public:
   ...
   void decreaseKey(int pos, int val)// Decreases distance in pos to val
   {
      array[pos]->distance = val; 
      upHeap(pos);
   }; 
...
private:
   void downHeap(int root){// starting with element in position int, sift it down the heap 
      ...
      do{
	      if (child < (elements-1)){
		      if ( array[child+1]->distance < array[child]->distance ) child++; // pick smaller
         }	   
	      if ( array[child]->distance < item->distance ){//swap
   ...
   void upHeap(int new_pos){// starting with element in position int, sift it up the heap
      ...
      while((new_pos != 0) && (item->distance < array[parent]->distance)) //loop while the item has not become the main root, and while its value is less than its parent
      {
