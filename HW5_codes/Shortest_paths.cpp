...
/* For simplicity, input vertices are numbered from 1 */
  ...
  string inputfile = "smallgraph.txt";
  ...
  Or = 1; // origin node
  ofstream outfile(inputfile+"_out");
...
//Reset for next algorithm 
  ...
  cout << "CALLING Bellman-Ford Simple \n" << endl;
  ...
  cout << "CALLING Bellman-Ford with work Queue \n" << endl;
  outfile << "Bellman-Ford_workQueue\n"<< endl;
  start = chrono::steady_clock::now();
  BellmanFord_Queue(Nodes,Or,Nm);
  stop = chrono::steady_clock::now();
  difference_in_time = stop - start;
  difference_in_seconds = double(difference_in_time.count());
  outfile << "CPU TIME in SECS " << difference_in_seconds<<endl;
  ...
  for (int i=0;i<=Nm;i++){
    ...
    cout << "CALLING Bellman-Ford with standard libary Queue \n" << endl;
    outfile << "Bellman-Ford-std Queue\n"<< endl;
    start = chrono::steady_clock::now();
    BellmanFord_stdQueue(Nodes,Or,Nm);
    stop = chrono::steady_clock::now();
    difference_in_time = stop - start;
    difference_in_seconds = double(difference_in_time.count());
    outfile << "CPU TIME in SECS " << difference_in_seconds<<endl;
